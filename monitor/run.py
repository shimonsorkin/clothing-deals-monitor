"""Orchestrator: collect deals -> Telegram alerts + landing page.

Per store:
  * first run (no prior state)  -> seed silently, send one summary line
  * later runs                  -> alert only on new or further-dropped items
The landing page always reflects the full current set across all stores.
"""
from __future__ import annotations

import html

from .adapters import shopify
from .adapters.base import Deal
from .config import load_config
from .site import render
from .state import load_state, save_state
from .telegram import send

ADAPTERS = {"shopify": shopify}
MAX_ALERTS_PER_RUN = 40


def _deal_message(d: Deal) -> str:
    lines = [
        f"🛍 <b>{html.escape(d.store)}</b>",
        html.escape(d.product_title),
        f"Размер: <b>{html.escape(d.size)}</b>",
    ]
    pct = d.discount_pct
    if pct:
        lines.append(
            f"💸 <b>{html.escape(d.money(d.price))}</b> "
            f"<s>{html.escape(d.money(d.compare_at_price))}</s> (−{pct}%)"
        )
    else:
        lines.append(f"Цена: <b>{html.escape(d.money(d.price))}</b>")
    lines.append(f'<a href="{html.escape(d.url)}">Открыть товар →</a>')
    return "\n".join(lines)


def main() -> None:
    cfg = load_config()
    size_tokens = cfg["size_tokens"]
    state = load_state()
    all_deals: list[Deal] = []

    for store in cfg["stores"]:
        if not store.get("enabled", True):
            continue
        adapter = ADAPTERS.get(store.get("adapter"))
        if adapter is None:
            print(f"[{store.get('name')}] unknown adapter: {store.get('adapter')}")
            continue

        name = store["name"]
        try:
            deals = adapter.collect(store, size_tokens)
        except Exception as exc:  # one bad store must not sink the run
            print(f"[{name}] adapter error: {exc}")
            # keep last-known page content for this store: reuse nothing,
            # simply skip — page is rebuilt from whatever did succeed.
            continue

        all_deals.extend(deals)
        prev = state.get(name, {})
        first_run = not prev
        current: dict[str, int] = {}
        fresh: list[Deal] = []
        for d in deals:
            seen_price = prev.get(d.key)
            if seen_price is None or d.price < seen_price:
                fresh.append(d)
            current[d.key] = min(d.price, seen_price) if seen_price else d.price
        state[name] = current

        if first_run:
            send(
                f"✅ <b>{html.escape(name)}</b>: мониторинг включён. "
                f"Сейчас в твоём размере со скидкой: <b>{len(deals)}</b>. "
                f"Дальше — только новое и подешевевшее."
            )
        else:
            for d in fresh[:MAX_ALERTS_PER_RUN]:
                send(_deal_message(d))
            if len(fresh) > MAX_ALERTS_PER_RUN:
                send(
                    f"…и ещё {len(fresh) - MAX_ALERTS_PER_RUN} новых в "
                    f"<b>{html.escape(name)}</b> (показал первые "
                    f"{MAX_ALERTS_PER_RUN})."
                )
        print(f"[{name}] deals={len(deals)} new={len(fresh)} first_run={first_run}")

    render(all_deals)
    save_state(state)


if __name__ == "__main__":
    main()
